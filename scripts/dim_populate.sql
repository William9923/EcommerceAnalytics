...
insert into staging.dim_payment (
	order_id,
	num_payment, 
	total_payment_value,
...
(
	select 
		...
		true as is_current_version
	from p join (select * from staging.dim_payment where is_current_version = TRUE) stg
...
update staging.dim_payment 
set is_current_version = false 
where staging.dim_payment.payment_id_surr in (
	select stg.payment_id_surr 
	from staging.dim_payment stg inner join p
